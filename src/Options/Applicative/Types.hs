...
data ParserPrefs = ParserPrefs
  { prefMultiSuffix :: OsString     -- ^ metavar suffix for multiple options
...
data OptProperties = OptProperties
  ...
  , propMetaVar :: OsString                 -- ^ metavariable for this option
  , propShowDefault :: Maybe OsString       -- ^ what to show in the help text as the default
...
instance Fail.MonadFail ReadM where
  fail v = readerError $ OsString.unsafeEncodeUtf v
...
newtype Completer = Completer
  { runCompleter :: OsString -> IO [OsString] }

-- | Smart constructor for a 'Completer'
mkCompleter :: (OsString -> IO [OsString]) -> Completer
...
newtype CompletionResult = CompletionResult
  { execCompletion :: OsString -> IO OsString }
...
optMetaVar :: Option a -> OsString
optMetaVar = propMetaVar . optProps

optShowDefault :: Option a -> Maybe OsString
