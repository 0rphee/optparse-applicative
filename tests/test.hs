...
{-# NOINLINE unsafeDecode #-}
unsafeDecode :: OsString -> String
unsafeDecode = Unsafe.unsafePerformIO . OsString.decodeUtf

lines :: OsString -> [OsString]
lines s = OsString.split lr (OsString.dropWhileEnd (== lr) s)
  where
    lr = OsString.unsafeFromChar '\n'
...
checkHelpTextWith ecode pprefs name p args = ioProperty $ do
  ...
  expected <- readFile $ "tests/" ++ unsafeDecode name ++ ".err.txt"
  return $ assertError result $ \failure ->
    let (msg, code) = renderFailure failure name
    in  (expected === unsafeDecode msg ++ "\n") .&&. (ecode === code)
...
prop_cmd_header = once $
  let i  = info (helper <*> Commands.sample) (header [osstr|foo|])
...
prop_show_default = once $
  ...
  in  assertError result $ \failure ->
    ...
    in  assertHasLine
        [osstr|  -n ARG                   set count (default: 0)|]
        msg
...
prop_alt_help = once $
  let p :: Parser (Maybe (Either OsString OsString))
      p = p1 <|> p2 <|> p3
      p1 = (Just . Left)
        <$> strOption ( long [osstr|virtual-machine|]
                     <> metavar [osstr|VM|]
                     <> help [osstr|Virtual machine name|] )
      p2 = (Just . Right)
        <$> strOption ( long [osstr|cloud-service|]
                     <> metavar [osstr|CS|]
                     <> help [osstr|Cloud service name|] )
...
prop_optional_help = once $
  let p :: Parser (Maybe (OsString, OsString))
      p = optional ((,)
                    <$> strOption ( long [osstr|a|]
                                    <> metavar [osstr|A|]
                                    <> help [osstr|value a|] )
                    <*> strOption ( long [osstr|b|]
                                    <> metavar [osstr|B|]
                                    <> help [osstr|value b|] ) )
      i = info (p <**> helper) idm
...
prop_optional_requiring_parens = once $
  let p = optional $
      ...
      result = run i [[osstr|--help|]]
  in assertError result $ \failure ->
    let text = head . lines . fst $ renderFailure failure [osstr|test|]
    in  [osstr|Usage: test [(-a|--a) (-b|--b)]|] === text
...
prop_nested_optional_help = once $
  let p :: Parser (OsString, Maybe (OsString, Maybe OsString))
      p = (,) <$>
          (strOption ( short (OsString.unsafeFromChar 'a')
                       <> long [osstr|a|]
                       <> metavar [osstr|A|]
                       <> help [osstr|value a|] ) ) <*>
          (optional
           ((,) <$>
            (strOption ( long [osstr|b0|]
                         <> metavar [osstr|B0|]
                         <> help [osstr|value b0|] ) ) <*>
            (optional (strOption ( long [osstr|b1|]
                                   <> metavar [osstr|B1|]
                                   <> help [osstr|value b1|] )))))
      i = info (p <**> helper) idm
...
prop_long_equals_doesnt_do_shorts = once $
  let p :: Parser OsString
      p = option str (   short (OsString.unsafeFromChar 'i')
                       <> help [osstr|integer value|])
      i = info (p <**> helper) fullDesc
      result = execParserPure (prefs helpLongEquals) i [[osstr|--help|]]
  in assertError result $ \failure ->
    let text = head . lines . fst $ renderFailure failure [osstr|test|]
    in  [osstr|Usage: test -i ARG|] === text

prop_nested_fun :: Property
prop_nested_fun = once $
  let p :: Parser (OsString, Maybe (OsString, Maybe OsString))
      p = (,) <$>
          (strOption (short (OsString.unsafeFromChar 'a') <> long [osstr|a|] <> metavar [osstr|A|])) <*>
          (optional
           ((,) <$>
            (strOption (short (OsString.unsafeFromChar 'b') <> long [osstr|b|] <> metavar [osstr|B|])) <*>
            (optional (strOption (short (OsString.unsafeFromChar 'c') <> long [osstr|c|] <> metavar [osstr|C|])))))
      i = info (p <**> helper) briefDesc
      result = run i [[osstr|--help|]]
  in assertError result $ \failure ->
    let text = head . lines . fst $ renderFailure failure [osstr|test|]
    in  [osstr|Usage: test (-a|--a A) [(-b|--b B) [-c|--c C]]|] === text

prop_nested_commands :: Property
prop_nested_commands = once $
  let p3 :: Parser OsString
      p3 = strOption (short (OsString.unsafeFromChar 'a') <> metavar [osstr|A|])
...
prop_drops_back_contexts = once $
  let p3 :: Parser OsString
      p3 = strOption (short (OsString.unsafeFromChar 'a') <> metavar [osstr|A|])
      p2 = subparser (command [osstr|b|] (info p3 idm)  <> metavar [osstr|B|])
      p1 = subparser (command [osstr|c|] (info p3 idm)  <> metavar [osstr|C|])
      p0 = (,) <$> p2 <*> p1
...
prop_help_on_empty_sub = once $
  let p3 :: Parser OsString
      p3 = strOption (short (OsString.unsafeFromChar 'a') <> metavar [osstr|A|] <> help [osstr|both commands require this|])
      p2 = subparser (command [osstr|b|] (info p3 idm)  <> metavar [osstr|B|])
      p1 = subparser (command [osstr|c|] (info p3 idm)  <> metavar [osstr|C|])
      p0 = (,) <$> p2 <*> p1
      i = info (p0 <**> helper) idm
...
prop_disambiguate_in_same_subparsers = once $
  let p0 = subparser (command [osstr|oranges|] (info (pure [osstr|oranges|]) idm) <> command [osstr|apples|] (info (pure [osstr|apples|]) idm) <> metavar [osstr|B|])
      i = info (p0 <**> helper) idm
...
prop_disambiguate_commands_in_separate_subparsers = once $
  let p2 = subparser (command [osstr|oranges|] (info (pure [osstr|oranges|]) idm)  <> metavar [osstr|B|])
      p1 = subparser (command [osstr|apples|] (info (pure [osstr|apples|]) idm)  <> metavar [osstr|C|])
      p0 = p1 <|> p2
      i = info (p0 <**> helper) idm
      result = execParserPure (prefs disambiguate) i [[osstr|orang|]]
  in  assertResult result ((===) [osstr|oranges|])
...
prop_fail_ambiguous_commands_in_same_subparser = once $
  let p0 = subparser (command [osstr|oranges|] (info (pure ()) idm) <> command [osstr|orangutans|] (info (pure ()) idm) <> metavar [osstr|B|])
      i = info (p0 <**> helper) idm
      result = execParserPure (prefs disambiguate) i [[osstr|orang|]]
  in  assertError result (\_ -> property succeeded)
...
prop_fail_ambiguous_commands_in_separate_subparser = once $
  let p2 = subparser (command [osstr|oranges|] (info (pure ()) idm)  <> metavar [osstr|B|])
      p1 = subparser (command [osstr|orangutans|] (info (pure ()) idm)  <> metavar [osstr|C|])
      p0 = p1 <|> p2
      i = info (p0 <**> helper) idm
      result = execParserPure (prefs disambiguate) i [[osstr|orang|]]
  in  assertError result (\_ -> property succeeded)
...
prop_without_disambiguation_same_named_commands_should_parse_in_order = once $
  let p3 = subparser (command [osstr|b|] (info (pure ()) idm)  <> metavar [osstr|B|])
      p2 = subparser (command [osstr|a|] (info (pure ()) idm)  <> metavar [osstr|B|])
      p1 = subparser (command [osstr|a|] (info (pure ()) idm)  <> metavar [osstr|C|])
      p0 = (,,) <$> p1 <*> p2 <*> p3
      i = info (p0 <**> helper) idm
...
prop_completion = once . ioProperty $
  let p = (,)
        <$> strOption (long [osstr|foo|] <> value OsString.empty)
        <*> strOption (long [osstr|bar|] <> value OsString.empty)
      i = info p idm
      result = run i [[osstr|--bash-completion-index|], [osstr|0|]]
  in case result of
    CompletionInvoked (CompletionResult err) -> do
      completions <- lines <$> err [osstr|test|]
      return $ [[osstr|--foo|], [osstr|--bar|]] === completions
...
prop_completion_opt_after_double_dash = once . ioProperty $
  let p = (,)
        <$> strOption (long [osstr|foo|] <> value OsString.empty)
        <*> argument readerAsk (completeWith [[osstr|bar|]])
      i = info p idm
...
prop_completion_only_reachable = once . ioProperty $
  let p :: Parser (OsString, OsString)
      p = (,)
        <$> strArgument (completeWith [[osstr|reachable|]])
        <*> strArgument (completeWith [[osstr|unreachable|]])
      i = info p idm
      result = run i [[osstr|--bash-completion-index|], [osstr|0|]]
  in case result of
    CompletionInvoked (CompletionResult err) -> do
      completions <- lines <$> err [osstr|test|]
      return $ [[osstr|reachable|]] === completions
    Failure _   -> return $ counterexample "unexpected failure" failed
    Success val -> return $ counterexample ("unexpected result " ++ show val) failed
...
prop_completion_only_reachable_deep = once . ioProperty $
  let p :: Parser (OsString, OsString)
      p = (,)
        <$> strArgument (completeWith [[osstr|seen|]])
        <*> strArgument (completeWith [[osstr|now-reachable|]])
      i = info p idm
      result = run i [ [osstr|--bash-completion-index|], [osstr|2|]
  ...
  in case result of
    CompletionInvoked (CompletionResult err) -> do
      completions <- lines <$> err [osstr|test|]
      return $ [[osstr|now-reachable|]] === completions
    Failure _   -> return $ counterexample "unexpected failure" failed
    Success val -> return $ counterexample ("unexpected result " ++ show val) failed
...
prop_completion_multi = once . ioProperty $
  let p :: Parser [OsString]
      p = many (strArgument (completeWith [[osstr|reachable|]]))
      i = info p idm
...
prop_completion_rich = once . ioProperty $
  let p = (,)
        <$> option readerAsk (long [osstr|foo|] <> help [osstr|Fo?|])
        <*> option readerAsk (long [osstr|bar|] <> help [osstr|Ba?|])
      i = info p idm
      result = run i [[osstr|--bash-completion-enriched|], [osstr|--bash-completion-index|], [osstr|0|]]
  in case result of
    CompletionInvoked (CompletionResult err) -> do
      completions <- lines <$> err [osstr|test|]
      return $ [[osstr|--foo\tFo?|], [osstr|--bar\tBa?|]] === completions
    Failure _   -> return $ counterexample "unexpected failure" failed
    Success val -> return $ counterexample ("unexpected result " ++ show val) failed
...
prop_completion_rich_lengths = once . ioProperty $
  let p = (,)
        <$> option readerAsk (long [osstr|foo|] <> help [osstr|Foo hide this|])
        <*> option readerAsk (long [osstr|bar|] <> help [osstr|Bar hide this|])
      i = info p idm
  ...
  in case result of
    CompletionInvoked (CompletionResult err) -> do
      completions <- lines <$> err [osstr|test|]
      return $ [[osstr|--foo\tFoo...|], [osstr|--bar\tBar...|]] === completions
    Failure _   -> return $ counterexample "unexpected failure" failed
    Success val -> return $ counterexample ("unexpected result " ++ show val) failed
...
prop_bind_usage = once $
  let p :: Parser [OsString]
      p = many (argument str (metavar [osstr|ARGS...|]))
      i = info (p <**> helper) briefDesc
      result = run i [[osstr|--help|]]
  in assertError result $ \failure ->
    let text = head . lines . fst $ renderFailure failure [osstr|test|]
    in  [osstr|Usage: test [ARGS...]|] === text
...
prop_issue_35 = once $
  let p =  flag' True (short (OsString.unsafeFromChar 't') <> hidden)
      ...
      result = run i []
  in assertError result $ \failure ->
    let text = lines . fst $ renderFailure failure [osstr|test|]
    in  [[osstr|Missing: -f|], OsString.empty, [osstr|Usage: test -f|]] === text
...
prop_error_context = once $
  ...
  in assertError result $ \failure ->
      ...
      in  conjoin [ counterexample "no context in error message (option)" ([osstr|port|] `OsString.isInfixOf` errMsg)
                  , counterexample "no context in error message (value)"  ([osstr|foo|] `OsString.isInfixOf` errMsg)]
  where
    pk :: Int -> Int -> (Int, Int)
    pk = (,)
...
prop_issue_47 = once $
  let p = option r (long [osstr|test|] <> value 9) :: Parser Int
      r = readerError [osstr|error message|]
      result = run (info p idm) [[osstr|--test|], [osstr|x|]]
  in assertError result $ \failure ->
    let text = head . lines . fst $ renderFailure failure [osstr|test|]
    in  counterexample "no error message" ([osstr|error message|] `OsString.isInfixOf` text)
...
prop_long_help = once $
  let p = Formatting.opts <**> helper
      i = info p
        ( progDesc (mconcat
            [ [osstr|This is a very long program description. |]
            , [osstr|This text should be automatically wrapped |]
            , [osstr|to fit the size of the terminal|] ]) )
  in checkHelpTextWith ExitSuccess (prefs (columns 50)) [osstr|formatting|] i [[osstr|--help|]]

prop_issue_50 :: Property
prop_issue_50 = once $
  let p = argument str (metavar [osstr|INPUT|])
...
prop_intersperse_1 = once $
  let p = many (argument str (metavar [osstr|ARGS|]))
...
prop_intersperse_2 = once $
  let p = subparser
          (  command [osstr|run|]
             ( info (many (argument str (metavar [osstr|OPTIONS|])))
                    noIntersperse )
          <> command [osstr|test|]
             ( info (many (argument str (metavar [osstr|ARGS|])))
                    idm ) )
      i = info p idm
...
prop_intersperse_3 = once $
  let p = (,,) <$> switch ( long [osstr|foo|] )
               <*> strArgument ( metavar [osstr|FILE|] )
               <*> many ( strArgument ( metavar [osstr|ARGS...|] ) )
...
prop_issue_52 = once $
  ...
  in assertError (run i []) $ \failure -> do
    let text = lines . fst $ renderFailure failure [osstr|test|]
    [[osstr|Missing: FOO|], OsString.empty, [osstr|Usage: test FOO|]] === text

prop_multiple_subparsers :: Property
prop_multiple_subparsers = once $
  let p1 = subparser
        (command [osstr|add|] (info (pure ())
             ( progDesc [osstr|Add a file to the repository|] )))
      p2 = subparser
        (command [osstr|commit|] (info (pure ())
             ( progDesc [osstr|Record changes to the repository|] )))
...
prop_reader_error_mplus = once $
  ...
  in assertError (run i [[osstr|foo|]]) $ \failure ->
    let text = head . lines . fst $ renderFailure failure [osstr|test|]
    in  [osstr|foo /= 42|] === text

prop_missing_flags_described :: Property
prop_missing_flags_described = once $
  let p :: Parser (OsString, OsString, Maybe OsString)
      p = (,,)
       <$> option str (short (OsString.unsafeFromChar 'a'))
       <*> option str (short (OsString.unsafeFromChar 'b'))
       <*> optional (option str (short (OsString.unsafeFromChar 'c')))
      i = info p idm
  in assertError (run i [[osstr|-b|], [osstr|3|]]) $ \failure ->
    let text = head . lines . fst $ renderFailure failure [osstr|test|]
    in  [osstr|Missing: -a ARG|] === text
...
prop_many_missing_flags_described = once $
  ...
  in assertError (run i []) $ \failure ->
    let text = head . lines . fst $ renderFailure failure [osstr|test|]
    in  [osstr|Missing: -a ARG -b ARG|] === text

prop_alt_missing_flags_described :: Property
prop_alt_missing_flags_described = once $
  let p :: Parser OsString
      p = option str (short (OsString.unsafeFromChar 'a')) <|> option str (short (OsString.unsafeFromChar 'b'))
      i = info p idm
  in assertError (run i []) $ \failure ->
    let text = head . lines . fst $ renderFailure failure [osstr|test|]
    in  [osstr|Missing: (-a ARG | -b ARG)|] === text
...
prop_missing_option_parameter_err = once $
  let p :: Parser OsString
      p = option str (short (OsString.unsafeFromChar 'a'))
      i = info p idm
  in assertError (run i [[osstr|-a|]]) $ \failure ->
    let text = head . lines . fst $ renderFailure failure [osstr|test|]
    in  [osstr|The option `-a` expects an argument.|] === text
...
prop_many_pairs_lazy_progress = once $
  let p :: Parser [(Maybe OsString, OsString)]
      p = many $ (,) <$> optional (option str (short (OsString.unsafeFromChar 'a'))) <*> argument str idm
      i = info p idm
...
prop_suggest = once $
  ...
  in assertError result $ \failure ->
    ...
    in  counterexample (unsafeDecode msg)
       $  OsString.isInfixOf [osstr|Did you mean one of these?
    first
    fst|] msg
...
prop_grouped_some_option_ellipsis = once $
  let x :: Parser OsString
      x = strOption (short (OsString.unsafeFromChar 'x') <> metavar [osstr|X|])
      p = prefs (multiSuffix [osstr|...|])
...
prop_grouped_some_dual_option_ellipsis = once $
  let x :: Parser OsString
      x = strOption (short (OsString.unsafeFromChar 'a') <> short (OsString.unsafeFromChar 'b') <> metavar [osstr|X|])
      p = prefs (multiSuffix [osstr|...|])
      r = show . extractChunk $ H.briefDesc p (x *> some x)
  in r === "(-a|-b X) (-a|-b X)..."
...
prop_issue_450_subcommand_show_help_on_empty_inline = once $
  let
    q = (,)
      <$> flag' () (short (OsString.unsafeFromChar 'a') <> help [osstr|supply a|])
      <*> flag' () (short (OsString.unsafeFromChar 'b') <> help [osstr|supply b|])

    p =
      subparser $
        command [osstr|foo|] $ info q $
        progDesc [osstr|Foo commands.|]

    i = info (p <**> helper) briefDesc
    result = execParserPure (prefs (showHelpOnEmpty <> subparserInline)) i [[osstr|foo|]]
  in assertError result $ \failure ->
    let text = lines . fst $ renderFailure failure [osstr|test|]
    in [[osstr|Usage: test foo -a -b|]
       ,OsString.empty
       ,[osstr|  Foo commands.|]
       ,OsString.empty
       ,[osstr|Available options:|]
       ,[osstr|  -a                       supply a|]
       ,[osstr|  -b                       supply b|]] === text
...
prop_issue_450_ensure_missing_still_shows = once $
  ...
  in assertError result $ \failure ->
    ...
    in [[osstr|Missing: -b|]
       ,OsString.empty
       ,[osstr|Usage: test foo -a -b|]
       ,OsString.empty
       ,[osstr|  Foo commands.|]] === text
...
prop_stringChunk_1 :: Positive Double -> Positive Int -> OsString -> Property
prop_stringChunk_1 (Positive f) (Positive w) s =
  equalDocs f w (extractChunk (stringChunk s))
                (Doc.pretty $ unsafeDecode s)

prop_stringChunk_2 :: OsString -> Property
prop_stringChunk_2 s = isEmpty (stringChunk s) === OsString.null s

prop_paragraph :: OsString -> Property
prop_paragraph s = isEmpty (paragraph s) === null (OsString.splitWith (isSpace . OsString.toChar) s)
...
instance Arbitrary OsString where
  arbitrary = (OsString.unsafeEncodeUtf . getPrintableString) <$> (arbitrary :: Gen PrintableString)
